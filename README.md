# 👋 Hi, I'm Theon Anabel!

🎓 **Informatics Student** at Universitas Multimedia Nusantara  
💻 Passionate about **Software Development**, **Web Technology**, and **AI**.  
🌱 Currently learning **Machine Learning**, **Data Science**, and **System Design**.  

---

### 🧠 Skills

**Programming Languages:**
- Python 🐍
- C 
- Java
- Kotlin
- JavaScript
- SQL
- PHP

**Tools & Frameworks:**
- Android Studio
- Firebase
- RStudio  
- Jupyter
- Laravel 
- React  
- Git & GitHub  

---

- 📱 **[My Mobile App Project](https://github.com/P0Y0Y0/LifeBalance)** — an android mobile app built with Kotlin and Firebase  
- 🌐 **[Laravel Web App](https://github.com/P0Y0Y0/UAS---Webprog---Sem3---Website_PerpustakaanKelapaDua)** — a web application built using Laravel and MySQL  

---

### 📊 GitHub Stats

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=P0Y0Y0&layout=compact&theme=radical)

---

### 📫 Connect with Me

- [![LinkedIn Badge](https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat&logo=LinkedIn&logoColor=white)](https://id.linkedin.com/in/theon-anabel-302395147)
- [![Gmail Badge](https://img.shields.io/badge/-Gmail-D14836?style=flat&logo=Gmail&logoColor=white)](mailto:theonanabel15@gmail.com)
- [![Instagram Badge](https://img.shields.io/badge/-Instagram-E4405F?style=flat&logo=Instagram&logoColor=white)](https://instagram.com/theon.anabel)

---

⭐ _“Code is like humor. When you have to explain it, it’s bad.” — Cory House_
